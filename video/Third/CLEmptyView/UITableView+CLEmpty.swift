...
extension UITableView {
   ...
   public func normalEmptyView(){
        var loadingArray:[String] = [String]()
        for i in 0...29 {
            loadingArray.append("loading_\(i)")
        }
        config.clEmptyView.addEmptyImage(imageNmae: "empty")
            .addEmptyTis(tips: NSAttributedString(string: "加载内容失败,点击重试"))
            .addLoadingImage(imageNames: loadingArray)
    ...
    public func standardEmptyView(){
        config.clEmptyView
            .addEmptyImage(imageNmae: "emptySetHolder")
            .addEmptyTis(tips: NSAttributedString(string: "加载内容失败,点击重试"))
